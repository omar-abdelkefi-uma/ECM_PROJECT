...
export class EditAdministratorComponent implements OnInit {
  ...
  typeuser: string;
  constructor(private route: ActivatedRoute, private administratorservice: AdminService, private roleservice: RoleService, private router: Router, private _formBuilder: FormBuilder) {
    7
  }
  ngOnInit(): void {
  ...
  reloadData() {
    ...
    this.administratorservice.getadministrator(this.id).subscribe(data => {
      ...
      this.experiences = this.administrator.experiences;
      this.documents = this.administrator.documents;
      ...
      if (this.administrator.imageprofile.imageid && this.images.length === 0) {
        this.img = new Alias(
          this.i,
          { // modal
            img: 'data:image/jpg;base64,' + this.administrator.imageprofile.imageid
          });
        this.imagesRect.push(this.img);
      }
  ...
  update() {
    ...
    debugger;
    this.administratorservice.updateadministrator(this.administrator).subscribe(
      data => this.router.navigate(["/alluser/administrator"]),
  ...
  addimage() {
    this.router.navigate(['/alluser/addimage', this.administrator.id], { queryParams: { type: "admin" } });
  }
